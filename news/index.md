...
- added network names for sampson dataset
  ([\#10](https://github.com/schochastics/networkdata/issues/10))
...
- fixed error in `ffe_*` data (Freeman’s page is down, so the new
  version comes from [here](https://github.com/zalmquist/networkdata))
...
- fixed genders in `crime`
  ([\#5](https://github.com/schochastics/networkdata/issues/5))
...
- `show_networks()` was returning a tibble which requires additional
  dependencies. The function now returns a data.frame
